...
int BVH::extendBoundingBox(BoundingBox* target, BoundingBox* source) {
    int extend_count = 0;
...
void BVH::generateBVH(std::vector<Vector3>& vertices, std::vector<Surface*> surfaces) {
    ...
    root = new BVHNode(*bb, surfaces);

    divide(root);

}

bool BVH::hits_before_1(Ray& ray) {
...
bool BVH::rayTrace(Ray& ray, int depth, Scene& currentScene, Color& color, double min_t) {
    std::stack<BVHNode*> stk;
    stk.push(root);
    struct RayHitInfo hitinfo;
